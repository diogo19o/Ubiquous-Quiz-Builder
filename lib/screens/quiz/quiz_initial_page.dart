...
class QuizInitialPage extends StatelessWidget {
      ...
      child: RichText(
        text: TextSpan(
          ...
          children: <TextSpan>[
            ...
            TextSpan(
                text: tituloDetalhe == "Criador"
                    ? _services.getUserById(valorDetalhe)
                    : tituloDetalhe == "Núm. Perguntas" ? _services.getNumberOfQuestions(valorDetalhe).toString() : valorDetalhe,
                style: TextStyle(color: AppColors.SecondaryLight, fontSize: 20)),
          ],
        ),
      ),
    );
  ...
  void _navigateToQuiz(BuildContext context) {
    if (_dataSource.questionarioAtivo.questionarioDetails.modo ==
        "questionario") {
      if (_services.jaRespondeuQuestionario()) {
        scaffoldKey.currentState.showSnackBar(
            SnackBar(content: Text("Já respondeu a este questionário!")));
      } else {
        Get.to(() => QuizScreen());
      }
    } else {
  ...
  Stack buildBody(BuildContext context,
      QuestionarioDetails detalhesQuestionario, Size size) {
    return Stack(
      children: [
        SafeArea(
          child: Padding(
            padding: const EdgeInsets.symmetric(horizontal: kDefaultPadding20),
            child: Column(
                    ...
                    children: [
                      ...
                      Text("${detalhesQuestionario.titulo}:",
                          style: Theme.of(context)
                              ...
                              .copyWith(color: AppColors.SecondaryMid)),
                      SizedBox(height: 20),
                      !(detalhesQuestionario.modo == "questionario")
                          ? _buildTextDetalhe(context, "Dificuldade",
                              detalhesQuestionario.dificuldade)
                          : SizedBox(height: 0),
                      _buildTextDetalhe(
                          context, "Descrição", detalhesQuestionario.descricao),
                      _buildTextDetalhe(context, "Núm. Perguntas",
                          detalhesQuestionario.id.toString()),
                      _buildTextDetalhe(context, "Criador",
                          detalhesQuestionario.IDuserCriador),
